...
if not Fluent then
    warn("[Apex] Fluent failed to load (check internet connection)")
end

-- State store (same semantics as the old standalone UI)
local dt = {}
dt.__state = {}
dt.__callbacks = {}
...
function r.notify(title, content, kind, duration)
    pcall(function()
        Fluent:Notify({
            Title = tostring(title or "Apex Hub"),
...
-- status row shim (used by the dashboard refresh below)
...
local function addSlider(parent, opt)
    ...
    local rounding = (step % 1) ~= 0 and 1 or 0
    local unit = tostring(opt.Suffix or ""):match("^%s*(.-)%s*$") or ""
    local title = opt.Title or "Slider"
    if unit ~= "" then title = title .. " (" .. unit .. ")" end
...
local function addDropdown(parent, opt)
    ...
    if isMulti then
        ...
        if typeof(opt.Default) == "table" then
            for _, val in ipairs(opt.Default) do sel[val] = true end
        end
        local list = {}
...
local function addInput(parent, opt)
    local id = opt.Id
    dt.__state[id] = tostring(opt.Default or "")
    return parent:AddInput(id, {
        Title = opt.Title,
        Description = opt.Description,
        Placeholder = opt.Placeholder or "",
        Default = dt.__state[id],
        Callback = function(v)
            local out = tostring(v or "")
            dt.SetState(id, out, true)
            if opt.Callback then pcall(opt.Callback, out) end
        end,
    })
end
...
if Window then
    ...
    addToggle(secFuse, { Id = "AutoFusePets", Title = "Auto Fuse Pets", Default = false })
    addDropdown(secFuse, { Id = "FuseRarities", Title = "Fuse Rarities", Options = at, Multi = true, Default = {} })
    addDropdown(secFuse, { Id = "FuseMutations", Title = "Fuse Mutations", Options = av, Multi = true, Default = {} })
    addDropdown(secFuse, { Id = "FuseTarget", Title = "Pick Group By", Options = ax, Default = "Highest Rarity" })
    addToggle(secFuse, { Id = "FuseKeepMutated", Title = "Never Fuse Mutated", Default = true })
    addToggle(secFuse, { Id = "FuseKeepEquipped", Title = "Never Fuse Equipped", Default = true })
    addToggle(secFuse, { Id = "FuseAutoReveal", Title = "Auto Complete Reveal", Default = true })
    addSlider(secFuse, { Id = "FuseMaxScale", Title = "Maximum Scale to Fuse", Min = 0, Max = 10, Default = 10, Step = 0.1 })
    addSlider(secFuse, { Id = "FuseKeepPerCategory", Title = "Keep Per Pet Type", Min = 0, Max = 20, Default = 0, Step = 1 })
    addSlider(secFuse, { Id = "FuseInterval", Title = "Fuse Interval", Min = 1, Max = 120, Default = 8, Step = 1, Suffix = "s" })
    ...
    addSlider(secSellEggs, { Id = "SellEggInterval", Title = "Sell Interval", Min = 1, Max = 120, Default = 8, Step = 1, Suffix = "s" })

    addToggle(secSellPets, { Id = "AutoSellPets", Title = "Auto Sell Pets", Default = false })
    addDropdown(secSellPets, { Id = "SellRarities", Title = "Sell Rarities", Options = at, Multi = true, Default = {} })
    addDropdown(secSellPets, { Id = "SellMutations", Title = "Sell Mutations", Options = av, Multi = true, Default = {} })
    addToggle(secSellPets, { Id = "SellKeepMutated", Title = "Never Sell Mutated", Default = true })
    addToggle(secSellPets, { Id = "SellKeepEquipped", Title = "Never Sell Equipped", Default = true })
    addSlider(secSellPets, { Id = "SellMaxScale", Title = "Maximum Scale to Sell", Min = 0, Max = 10, Default = 10, Step = 0.1 })
    ...
    addToggle(secEsp, { Id = "EspGuards", Title = "Guard ESP", Default = false })
    addToggle(secEsp, { Id = "EspPets", Title = "Pet ESP", Default = false })
    addToggle(secEsp, { Id = "EspPlayers", Title = "Player ESP", Default = false })
    addToggle(secEsp, { Id = "EspMachines", Title = "Machine ESP", Default = false })
    addToggle(secEsp, { Id = "EspPlots", Title = "Plot ESP", Default = false })
    addSlider(secEsp, { Id = "EspDistance", Title = "Render Distance", Min = 100, Max = 6000, Default = 2000, Step = 50, Suffix = "studs" })
    addToggle(secMove, { Id = "WalkSpeedEnabled", Title = "Walk Speed Override", Default = false })
    addSlider(secMove, { Id = "WalkSpeed", Title = "Walk Speed", Min = 16, Max = 500, Default = 32, Step = 1 })
    addToggle(secMove, { Id = "JumpPowerEnabled", Title = "Jump Power Override", Default = false })
    ...
    addSlider(secMove, { Id = "FlySpeed", Title = "Fly Speed", Min = 10, Max = 400, Default = 60, Step = 1 })
    addDropdown(secTp, { Id = "WaypointTarget", Title = "Waypoint", Options = dq, Default = "Base" })
    addButton(secTp, { Title = "Teleport to Waypoint", Callback = function()
        task.spawn(function()
    ...
    addToggle(secSes, { Id = "AutoReconnect", Title = "Auto Reconnect", Default = false })
    addButton(secSes, { Title = "Rejoin Server", Callback = function() r.rejoinServer() end })
    addButton(secSes, { Title = "Copy Join Script", Callback = function()
        pcall(function() setclipboard(string.format(
            'game:GetService("TeleportService"):TeleportToPlaceInstance(%d, "%s", game:GetService("Players").LocalPlayer)',
            game.PlaceId, bs)) end)
        r.notify("Copied", "Join script copied", "Success", 3)
    end })
    addToggle(secPerf, { Id = "FpsBoost", Title = "FPS Boost", Default = false,
        Callback = function(gt) if gt then r.enableFpsBoost() else r.disableFpsBoost() end end })
    addToggle(secPerf, { Id = "DisableRendering", Title = "Disable 3D Rendering", Default = false,
        Callback = function(gt) r.applyRendering(gt) end })
    addSlider(secPerf, { Id = "FpsCap", Title = "FPS Cap", Min = 15, Max = 360, Default = 60, Step = 1, Suffix = "fps",
        Callback = function(gt) r.applyFpsCap(gt) end })
    addToggle(secWeb, { Id = "WebhookEnabled", Title = "Enable Webhooks", Default = false })
    addInput(secWeb, { Id = "WebhookUrl", Title = "Webhook URL", Placeholder = "https://discord.com/api/webhooks/...", Default = "" })
    addInput(secWeb, { Id = "WebhookPingId", Title = "Ping User ID", Placeholder = "123456789012345678", Default = "" })
    addSlider(secWeb, { Id = "WebhookInterval", Title = "Summary Interval", Min = 1, Max = 180, Default = 15, Step = 1, Suffix = "min" })
    addToggle(secWeb, { Id = "WebhookEggSpawns", Title = "List Spawned Eggs", Default = true })
    addDropdown(secWeb, { Id = "WebhookRarities", Title = "Rarities", Options = at, Multi = true, Default = {} })
    addToggle(secWeb, { Id = "WebhookDisconnectAlerts", Title = "Disconnect Alerts", Default = false })
    addButton(secWeb, { Title = "Send Summary Now", Callback = function()
        task.spawn(function()
            local gt = r.sendSummary()
            r.notify("Webhook", gt and "Sent" or "Failed", gt and "Success" or "Error", 3)
        end)
    end })
    addParagraph(secAbout, { Title = "Script Dev", Content = "Apex" })
    addParagraph(secAbout, { Title = "UI", Content = "Fluent" })
    addParagraph(secAbout, { Title = "Discord", Content = n })
    addButton(secAbout, { Title = "Copy Discord Link", Callback = function()
...
function r.unload()
    ...
    r.clearTable(br)
    pcall(function() if Fluent then Fluent:Destroy() end end)
    pcall(function() if getgenv then getgenv().Fluent = nil end end)
